...
class Home extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->bpl_home = $this->api_url . '&start=0&limit=5&content_types=news&pages=710&content_tags=959';
        /*$this->live_home_c = $this->api_url . 'special_filter=latest&ref=402751';
        $this->live_home_content = $this->detailed_article . '402751';*/
...
/*            $data["live_home_content"] = $bt_home_page_life_busi['live_home_content'];
    ...
    public function urlRewrite($seg, $type = NULL, $id = NULL, $slug = NULL) {
        //header("Location: http://www.banglatribune.com$_SERVER[REQUEST_URI]/?desktop=1");
        if (!empty($seg)) {
            if ($this->uri->total_segments() == 1) {
                ...
                if($this->uri->segment(1) == 'lives'){
                    if(!$cache_live){
                        ...
                        if (!isset($data['details_live']['error'])) {
                            $data['pagetitle'] = $data['details_live']['title'];
                            $this->cache->memcached->save('bt_live_news_' . $id, $data, 40);
                        } else {
                            $data['error'] = $data['details_live']['error'];
                        }
                    } else {
                        $data['details_live'] = $cache_live['details_live'];
    ...
    function test() {
        ...
        if (!$bt_home_page_life_busi) {
            ...
            $data_three["politics_home"] = $this->common->doCURLRequest($this->politics_home, $this->APP_KEY);
            sleep(1);
            $data_three["national_election"] = $this->common->doCURLRequest($this->national_election, $this->APP_KEY);
            sleep(1);
//            $data_three["university_home_1"] = $this->common->doCURLRequest($this->university_home_1, $this->APP_KEY);
//            sleep(1);
//            $data_three["university_home_2"] = $this->common->doCURLRequest($this->university_home_2, $this->APP_KEY);
//            sleep(1);
//            $data_three["university_home_3"] = $this->common->doCURLRequest($this->university_home_3, $this->APP_KEY);
//            sleep(1);
//            $data_three["university_home_4"] = $this->common->doCURLRequest($this->university_home_4, $this->APP_KEY);
//            sleep(1);
//            $data_three["university_home_5"] = $this->common->doCURLRequest($this->university_home_5, $this->APP_KEY);
            $this->cache->memcached->delete('bt_home_page_life_busi');
            $this->cache->memcached->save('bt_home_page_life_busi', $data_three);
            array_push($data, $data_three);
        } else {
            ...
            $data["journey_home"] = $bt_home_page_life_busi['journey_home'];
            $data["politics_home"] = $bt_home_page_life_busi['politics_home'];
            $data["national_election"] = $bt_home_page_life_busi['national_election'];
//            $data["university_home_1"] = $bt_home_page_life_busi['university_home_1'];
//            $data["university_home_2"] = $bt_home_page_life_busi['university_home_2'];
//            $data["university_home_3"] = $bt_home_page_life_busi['university_home_3'];
//            $data["university_home_4"] = $bt_home_page_life_busi['university_home_4'];
//            $data["university_home_5"] = $bt_home_page_life_busi['university_home_5'];
        }
        $data['features'] = $this->features;
        $data['active'] = 'home';
