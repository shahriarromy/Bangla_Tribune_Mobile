...
class Api_en extends CI_Controller {
    private $api_url;
    private $call_url;
    ...
    public function api_pages() {
        ...
        $cache_keys = 'bt_en_cache_pages';
        if ($machine_code == 'romy_dosto_emran') {
            $caches = $this->cache->memcached->get($cache_keys);
            if (!$caches) {
                $url = 'http://en.banglatribune.com/api/mobile_api/get_pages?APP_ID=1';
    ...
    public function format_image_url($response = array()) {
        foreach ($response['data'] as $key => $value) {
            ...
            $response['data'][$key]['web_url'] = $response['data'][$key]['web_url'].','.'http://m.banglatribune.com/api_en/api_single_page_template?content_id=' . $response['data'][$key]['content_id'];
        }
        return $response;
    }
    ...
    public function api_catagory_pages() {
        ...
        if ($machine_code == 'romy_dosto_emran') {
            $cache_keys = 'bt_en_cache_category_pages_';
            $post_data = array();
            $post_data['APP_KEY'] = '2theWorldwar';
            $url = 'http://en.banglatribune.com/api/mobile_api/get_contents?APP_ID=1';
    ...
    public function api_single_page_template() {
        ...
        //if ($machine_code == 'romy_dosto_emran') {
        $cache_keys = 'bt_en_cache_details_app_pages_' . $content_id;
        $caches = $this->cache->memcached->get($cache_keys);
        if (!$caches) {
            $url = 'http://en.banglatribune.com/api/mobile_api/get_contents?APP_ID=1&content_id=' . $content_id;
            $post_data['APP_KEY'] = '2theWorldwar';
            $response = curl_api_call(
                    array(
                        'url' => $url,
        ...
        $this->load->view('mobile_single_en', $data);
        //} else {
        /* $data['error'] = 'Invalid key';
          header('Content-Type: application/json');
          echo json_encode($data, JSON_PRETTY_PRINT); */
        ...
        if ($machine_code == 'romy_dosto_emran') {
            $data['tabs'] = array(
                array(
                    'id' => '1',
                    'title' => 'Featured',
                    'api_urls' => 'http://m.banglatribune.com/api_en/api_catagory_pages',
                    'content_types' => 'news',
                    'content_tags' => '',
